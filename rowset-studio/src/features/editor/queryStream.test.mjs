...
test('display bytes are bounded even with newline-terminated frames', async () => {
  await assert.rejects(readQueryStream(new Response(' '.repeat(32*1024*1024+1)+'\n')),/32 MB display limit/);
  await assert.rejects(readQueryStream(new Response(' '.repeat(16*1024*1024+1)+'\n')),/16 MB display limit/);
});
